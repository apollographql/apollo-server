...
### `modules`
In Apollo Server 3 the `modules` constructor option enabled you to specify your schema as an array of typeDefs and resolvers objects. Under the hood it used a a schema-making function from `graphql-tools`.

We recommend replacing any usage of `modules` using the `makeExecutableSchema` function:
```js
  new ApolloServer({
