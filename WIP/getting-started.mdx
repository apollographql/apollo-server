...
### Set up TypeScript
Next we'll set up and configure TypeScript to compile our code. We'll install `typescript`, 'nodemon', and `concurrently` so we compile our run our code simultaneously:
...
Finally we'll add a script to the `package.json` file you previously created:
...
standaloneServer(apolloServerInstance)
  .listen({ port: 4000 })
