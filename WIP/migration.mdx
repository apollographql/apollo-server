...
- More consistent typing for TS users, including typing for `context`.
...
Apollo Server 3 includes core-maintained bindings for many additional web frameworks and serverless environments.

These additional integrations are all maintained by the Apollo Server core team, who might not have the practical experience running servers that use most of these frameworks. These binding additionally  meant that every time the team wanted to add new features to Apollo Server very similar changes had to be made in every binding package, raising process friction and slowing down development.

These bindings meant:
  - Every time Apollo Server 3 has a change every integration also needed to be updated.
  - The Apollo Server core team was maintaining package they did not have practical experience using.
  - The majority of integrations has similar, but not identical API. Meaning that you has to know the syntax for the integration you were using at the time, leading to annoying and unnecessary syntax errors.
...
```ts
```
`ApolloServerStandalone`(our replacement for the batteries-included apollo-server package) will still link in `body-parser` and `cors`, but will not allow you configure those options.
...
### The `apollo-server` npm package is now `standaloneServer` function
...
In Apollo Server 4 we change this for clarity, `context` is the name of the "a function producing a context value:
...
If you are accessing the values of your context context value in a
plugin (e.g., on `GraphQLRequestContext` or as a named argument to
`willResolveField`) you should now use `contextValue`.

### We no  use Fetcher from `@apollo/utils.fetcher` instead of `apollo-server-env` for Fetch API TypeScript typing
...
Users will write 'PUBLIC' directly instead of CacheScope.Public. Note that if they are using TypeScript this is still perfectly type-safe (a typo of `UPBLIC` will fail to compile); and if they're not using TypeScript, well, `CacheScope.Upblic` doesn't crash Node either, so perhaps they should use TypeScript if they care about such things.
