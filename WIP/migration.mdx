...
We not use `context` for the function and `contextValue` for the
single value. The `contextValue` term matches the name used in
`graphql-js`.

The main place where users write this is providing a context function,
so that name won't change. If they are accessing the context value in a
plugin (on `GraphQLRequestContext` or a named argument to
`willResolveField`) it is now called `contextValue` there.
