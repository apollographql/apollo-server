...
There are a few high-level changes to how you work with framework integrations. There are more details on each of these changes in later sections.
...
If you used the "batteries included" `apollo-server` package in Apollo Server 3, you'll use the `startStandaloneServer` function in Apollo Server 4 instead.
...
is equivalent to this Apollo Server 4 code:
...
The `listen` option to `startStandaloneServer` replaces the arguments to the `server.listen()` function. (If you passed no arguments to `server.listen()`, you don't need to specify a `listen` option.) This object has the same type as the [options argument passed to Node's `net.Server.listen`](https://nodejs.org/api/net.html#serverlistenoptions-callback).
...
If you used the `apollo-server-express` package in Apollo Server 3, you'll use the `expressMiddleware` function in Apollo Server 4 (instead of using `server.applyMiddleware` or `server.getMiddleware`).
...
In Apollo Server 3, built in plugins such as `ApolloServerUsageReporting` are exported from `apollo-server-core` at the top level. You need to install this package in addition to the package (like `apollo-server` or `apollo-server-express`) from which you import `ApolloServer`.

In Apollo Server 3, the built in packages are in `@apollo/server`, the same npm package as `ApolloServer`. However, they are exported from deep exports. (Using deep imports means that you only have to evaluate the code associated with the plugins you actually use, and it's easier for JS bundlers to eliminate unused code.
...
You should also remove the dependency on `apollo-server-core` from your project.

TypeScript types associated with these plugins (such as `ApolloServerPluginUsageReportingOptions`) are exported from the same deep import as the plugin.
...
The one way that Apollo Server 3 treats `DataSource`s specially is that immediately after construction, it would invoke `dataSource.initialize({ cache, context })` on each `DataSource`. So in Apollo Server 4, you should just pass `cache` and `context` to the constructor yourself if you need them. In Apollo Server 4, you can find the `cache` as a new readonly field on the `ApolloServer` itself.

Let's say this is your code that uses a `RESTDataSource`-based `DataSource` with Apollo Server 3:
...
If you actually need access to the entire context value within your `DataSource` rather than just particular fields, you can make your context value be a `class`, which is allowed to refer to itself via `this` in its constructor:
...
In Apollo Server 3, the `mocks` and `mockEntireSchema` constructor options enabled Apollo Server to return simulated data for GraphQL operations based on your server's schema. Under the hood, Apollo Server 3's mocking functionality is provided via an outdated version of the [`@graphql-tools/mocks`](https://www.npmjs.com/package/@graphql-tools/mock) library.
...
The follow examples show different ways of using the `mocks` and `mockEntireSchema` constructor options in Apollo Server 3 on the left, and the corresponding way of using `@graphql-tools/mock` directly on the right. (Note that you can apply these changes while still using Apollo Server 3, if you'd like your upgrade to be more incremental.)
...
In Apollo Server 3, the `formatResponse` hook enables you to transform the structure of GraphQL response objects before they're sent to a client. This hook is called only after an operation executes (not if there is an error like a parse error that prevents execution). It receives the `response` `GraphQLResponse` and the `requestContext` (which does not yet contain the `response`). If it returns a non-null `GraphQLResponse`, that response is used instead of the one it received. This hook is provided as a top-level constructor argument, not as part of the plugin API.

There is also a `willSendResponse` plugin hook. This plugin hook is called at a similar place in the lifecycle to `formatResponse`. It receives a `requestContext`; the `GraphQLResponse` is on this object in the `response` field. It may mutate `requestContext.response`. It is called for all requests that get far enough to invoke `requestDidStart` (ie, any request where the JSON body can be properly parsed, etc). Anything that the `formatResponse` hook can do can also be accomplished with the `willSendResponse` plugin hook.

Apollo Server 4 removes the `formatResponse` hook. We instead recommend using the `willSendResponse` plugin to do the same thing. (Note that [the structure of `GraphQLResponse` has also changed](#graphqlresponse) so that most of its fields are nested under a `result` field.) You can tell if an operation made it to the execution phase by seeing if the result has a `data` field, so if it was important that your `formatResponse` hook is only called if execution occurred, you can check for the existence of this field. (There are some small edge cases here: for example, an error in the coercion of variable values would result in calling `formatResponse` but will not have `data` in the result. If differentiating these edge cases is important to you, open an issue and we'll help.)
...
your Apollo Server 4 code could use `willSendResponse` like this:
...
(Note that you should configure your health check to send a `apollo-require-preflight: true` request along with this method so that it is not blocked by the [CSRF prevention](/apollo-server/security/cors/#preventing-cross-site-request-forgery-csrf) security feature.)
