...
If your project still uses the `engine` option or the `ENGINE_`-prefixed environment variables, see [Migrating from the `engine` option](https://www.apollographql.com/docs/apollo-server/v2/migration-engine-plugins) before upgrading to Apollo Server 3.
