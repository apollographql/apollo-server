...
The `formatResponse` hook receives a successful operation's `response`, `GraphQLResponse`, and `requestContext` (which doesn't have a `response` yet). If the `formatResponse` hook returns a non-null `GraphQLResponse`, it uses that response instead of the initially received `response` argument. 


Apollo Server 4 removes the `formatResponse` hook. We instead recommend using the `willSendResponse` plugin hook, which enables you to do everything you previously did with `formatResponse`. The `willSendResponse` plugin hook receives an operation's `requestContext`, which has a `response` field containing the `GraphQLResponse` object. Note that the `willSendResponse` hook may mutate the `requestContext.response` field. 
