...
`CacheScope` is now exported from the new `@apollo/cache-control-types` package rather than from an Apollo Server package directly. This enables libraries that work with multiple GraphQL servers (such as `@apollo/subgraph`) to refer to it without needing to depend on all of `@apollo/server`.
...
The `CacheHint`, `CacheAnnotation`, `CachePolicy`, and `ResolveInfoCacheControl` types are now exported from the new `@apollo/cache-control-types` package rather than from an Apollo Server package directly. This enables libraries that work with multiple GraphQL servers (such as `@apollo/subgraph`) to refer to it without needing to depend on all of `@apollo/server`. (This package also contains `CacheScope` which as described above is now a TypeScript-only symbol.)

Apollo Server no longer uses the `declare module` TypeScript feature to declare that all `GraphQLResolveInfo` objects (ie, the `info` argument to resolvers) have a `cacheControl` field. Instead, `@apollo/cache-control-types` provides a `GraphQLResolveInfoWithCacheControl` interface that you can cast `info` to (if you don't want run-time validation), or functions `maybeCacheControlFromInfo` and `cacheControlFromInfo` if you do want runtime validation.
...
Alternatively, you can declare `info` to be of type `GraphQLResolveInfoWithCacheControl`; for example, if using `graphql-code-generator` with `typescript-resolvers`, you can use its `customResolveInfo` option.

(This is a TypeScript-specific change; the runtime representation has not changed, and JavaScript code can continue to access `info.cacheControl` directly.)
