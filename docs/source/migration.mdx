...
> If you set your API key but do _not_ set your graph ref or ID, then if you explicitly set up features like the usage reporting plugin or managed federation, Apollo Server 3 throws an error on startup. If you don't explicitly [set up or disable the usage reporting plugin](./api/plugin/usage-reporting/), Apollo Server 3 logs a warning suggesting that you either set your graph ref or disable the usage reporting plugin.
