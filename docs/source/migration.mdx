...
"Engine" is a previous name of [Apollo Studio](https://www.apollographql.com/docs/studio/). Prior to Apollo Server v2.18.0, you passed the `engine` constructor option to configure how Apollo Server communicates with Studio. One option for specifying your Apollo API key was the `ENGINE_API_KEY` environment variable; one way to specify your variant was the `ENGINE_SCHEMA_TAG` environment variable.
...
Note that if you are using Studio, you will also want to be sure to [set your graph ref or graph ID](#guessing-apollo-studio-graph-id-from-api-key).
...
In Apollo Server 2, if you specify an Apollo API key (eg, with the `APOLLO_KEY` environment variable) that starts with `service:your-graph-id:`, Apollo Server will automatically guess that your Studio graph ID (used for usage reporting, schema reporting, managed federation, and so on) is `your-graph-id`.

In Apollo Server 3, you should specify your Studio graph ID explicitly when using features that connect to Studio. You can specify the graph ID by itself in the `APOLLO_GRAPH_ID` environment variable (or via `new ApolloServer({apollo: {graphId}}))`, or alongside the variant in a string like `your-graph-id@your-graph-variant` in the `APOLLO_GRAPH_REF` environment variable (or via `new ApolloServer({apollo: {graphRef}})`). See [the `apollo` constructor option](./api/apollo-server/#apollo) for more details.

(If you set your API key but do not set your graph ref or ID, then if you explicitly set up features like the usage reporting plugin or managed federation, Apollo Server 3 will throw an error on startup; and if you don't explicitly [set up or disable the usage reporting plugin](./api/plugin/usage-reporting/), Apollo Server 3 will log a warning suggesting that you set your graph ref or disable the usage reporting plugin.)
