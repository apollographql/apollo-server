...
type DataSourcesFn<TDataSources extends DataSources = DataSources> =
  () => TDataSources;

interface ContextWithDataSources<TDataSources extends DataSources = DataSources>
  extends BaseContext {
  dataSources?: TDataSources;
}

export const ApolloDataSources = <
  TDataSources extends DataSources = DataSources,
>(options: {
  dataSources: DataSourcesFn<TDataSources>;
}): ApolloServerPlugin<ContextWithDataSources<TDataSources>> => ({
...
// 
const plugins = [ApolloDataSources({ dataSources })];
const server = buildServer({ schema, plugins });
```
