---
title: Migrating to Apollo Server 4 (Release Candidate)
---

> ⚠️ **Apollo Server 4 is currently available as a Release Candidate.** We believe the current version is suitable for use, but it is possible that breaking changes might occur before v4.0.0 is published for general availability. [Learn about release stages.](/resources/release-stages/#open-source-release-stages)
...
The Apollo Server 4 Release Candidate provides the following features:
...
> For a list of all breaking changes, see the [changelog](https://github.com/apollographql/apollo-server/blob/main/CHANGELOG.md) (not yet finished).
...
If you are using other Apollo Server 3 framework integration packages such as `apollo-server-koa` and `apollo-server-lambda`, your framework may now be supported as a community-supported integration. Take a look at [the integrations page](./integrations/integration-index) to see if a package exists for your framework, or help out by [building new integrations](./integrations/building-integrations) or [discussing how to maintain existing integrations](https://github.com/apollographql/apollo-server/labels/integration-collaborators) to ensure there is an Apollo Server 4 integration for your favorite framework.
...
> For Apollo Server 4, `apollo-datasource-rest` package has been updated and renamed [`@apollo/datasource-rest`](https://www.npmjs.com/package/@apollo/datasource-rest).
...
In Apollo Server 3, you could provide an initial [`context`](./data/resolvers#the-context-argument) to your resolvers by adding a `context` initialization function to the `ApolloServer` constructor:
