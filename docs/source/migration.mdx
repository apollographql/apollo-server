...
If you use Apollo Server with TypeScript, you must use TypeScript v4.7.0 or newer. (Apollo Server uses type system features introduced in v4.7. We would like to put ["downleveled"](https://github.com/sandersn/downlevel-dts) versions of the type definitions in `@apollo/server` as well for use with older versions of TypeScript, but have found using TypeScript's `typesVersions` feature [to be challenging](https://github.com/apollographql/apollo-server/issues/6423). If support for older versions of TypeScript is important to you and you would like to help us get `typesVersions` working, we'd appreciate PRs.)
...
> Note: The current alpha of Apollo Server does not work as an Apollo Gateway. We will fix this before the final v4.0.0 release. You can use this alpha to serve subgraphs, just not Gateways.
