...
// FIXME: this is a draft and makes unconfirmed assumptions; this info is completely TBD and made up by me with no input from outside opinions who also have a say in this.

Given that Apollo Server 5 is a nearly seamless upgrade, v4 will not receive security or bug fixes in the same way that we've handled them for v2 and v3 in the past. Apollo Server v4 will be end-of-life **May 31st, 2023**.
...
// FIXME: confirm / tweak assumptions

[Apollo Server 4](/apollo-server/v4/) is deprecated and will transition to end-of-life on **May 31st, 2023**. We encourage all Apollo Server 4 users to [**upgrade** to Apollo Server 5 now](./migration). The upgrade from v4 should take only a few minutes and require little to no changes.
