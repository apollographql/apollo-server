...
class AuthenticatedDataSource extends RemoteGraphQLDataSource {
  willSendRequest({ request, contextValue }) {
    request.http.headers.set('x-user-id', contextValue.userId);
  }
}
...
This method takes a `requestContext` object that contains both the original unmodified `request` and the current `contextValue`.
...
class CookieDataSource extends RemoteGraphQLDataSource {
  didReceiveResponse({ response, request, contextValue }) {
...
- The current operation's `contextValue`
...
[The `contextValue` object](../../data/resolvers/#the-context-argument) passed between the GraphQL operation's resolvers.
