...
This API reference documents the exports from the `@apollo/gateway` package. This package enables you to use Apollo Server as a gateway for a federated supergraph. For more information, see [Node.js gateway setup](/federation/building-supergraphs/router/#nodejs-gateway-setup).
...
example of using `ApolloGateway`, see [The graph router](/federation/building-supergraphs/router/).
...
[The `context` argument](/apollo-server/data/resolvers/#the-context-argument) passed between the GraphQL operation's resolvers.
