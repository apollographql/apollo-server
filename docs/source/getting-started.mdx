...
* [`@apollo/server`](https://www.npmjs.com/package/@apollo/server) is the main library for Apollo Server itself, which understands how to turn HTTP requests and responses into GraphQL operations, and runs them in an extensible context with support for plugins and other features.
...
5. Finally, replace the default `scripts` entry in your `package.json` file with these `type` and `scripts` entries:
...
6. You can now run `npm start`, which should successfully compile and run your empty `index.ts` file, printing something like the following:
...
<!-- TODO(AS4): Fix this sandbox link after docs examples is up, and make sure it matches the final version -->
...
TODO(AS4): Should we temporarily remove this while we don't have full docs?
