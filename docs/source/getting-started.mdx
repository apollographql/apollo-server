...
* [`@landingexp/apollo-server`](https://npm.im/apollo-server) is the core library for Apollo Server itself, which helps you define the shape of your data and how to fetch it.
...
```bash
npm install @landingexp/apollo-server graphql
```
...
```js:title=index.js
const { ApolloServer, gql } = require('@landingexp/apollo-server');
