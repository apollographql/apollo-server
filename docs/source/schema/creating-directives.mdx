...
Your schema can define custom directives that can then decorate _other_ parts of your schema. Apollo Server can execute custom logic whenever it encounters a particular directive in your schema, such as by transforming data returned by resolvers.
...
This example defines an `@upper` directive for this purpose:
...
This code _replaces_ the resolver of an `@upper` field with a new function. This new function first calls the _original_ resolver, then transforms its result to uppercase (assuming it's a string) before returning it.
