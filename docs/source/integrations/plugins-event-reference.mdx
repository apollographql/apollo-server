...
The `startupDidFail` hook is called once if server startup fails. This can occur if the schema fails to load or if a `serverWillStart` or `renderLandingPage` hook throws. This hook receives the `error` that was thrown, which is also the error that `await server.start()` will throw.
...
The `contextCreationDidFail` event is fired if the user-provided `context` function throws an error. Because the request context could not be fully created, this hook only receives the `error`, not a full `GraphQLRequestContext`; if you need more information about the request when handling this event, consider wrapping your `context` function in a `try`/`catch` and ensuring the appropriate information is added to the thrown error.
...
The `unexpectedErrorProcessingRequest` event is fired any time an "unexpected" error is thrown during request execution. "Unexpected" errors do not include more common errors like validation, parsing, or graphql execution errors. Rather, an unexpected error might occur when a plugin hook throws unexpectedly or a bug in Apollo Server is encountered. These errors indicate programming errors rather than surprising data from clients. Notably, when errors like this occur, the error is masked to the client. Note that this hook is on the top level of the plugin rather than on the object returned from `requestWillStart`, because this hook is invoked if `requestWillStart` throws.
