...
async function startApolloServer(typeDefs, resolvers) {
  ...
  await server.applyMiddleware({
    app,
    route: {            // shared route options for GET & POST requests
      cors: true
    },
    routeGet: {         // route options for GET requests
      description: 'Get Route Description'
    },
    routePost: {        // route options for POST requests
      description: 'Post Route Description',
      payload: {
...
`applyMiddleware` also accepts the following options:

* `routeGet` - GET specific route options
* `routePost` - POST specific route options
