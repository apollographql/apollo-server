...
> **Note:** All plugin lifecycle methods are `async`, _except for [`willResolveField`](./plugins-event-reference/#willresolvefield) and [`schemaDidLoadOrUpdate`](./plugins-event-reference/#schemadidloadorupdate)_.
...
<!-- TODO(AS4) Replace link once this ApolloServerPlugin on main --> 
> If you're using TypeScript to create a plugin, you can implement your plugin using the [`ApolloServerPlugin` interface](https://github.com/apollographql/apollo-server/blob/cc447c5687f67938efb976d7aa282f7a95e313db/packages/server/src/externalTypes/plugins.ts#L34).
