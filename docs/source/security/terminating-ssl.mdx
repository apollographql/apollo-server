...
{/* TODO (AS4) replace link with expressMiddleware article (once it exists) */}
If you're using Apollo Server in an application that must perform its _own_ SSL termination, you can use the `https` module with the [`expressMiddleware` function](../migration#migrate-from-apollo-server-express).
