...
Apollo Server's default CORS behavior enables anyone on the Internet to connect to the applications a server is serving up.
...
By default, Apollo Server doesn't allow you to pass cookies with requests.  If you want to use cookies for authentication, see [Passing Credentials with CORS](#passing-credentials-with-cors).
