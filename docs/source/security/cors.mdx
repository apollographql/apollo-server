...
> 📣 By default, Apollo Server 4 come with a feature protecting from CSRF and XS-Search attacks turned on. This CSRF prevention feature requires that any client sending operations via `GET` or multipart upload requests _must_ include a non-empty `Apollo-Require-Preflight` (or any other non-special-cased) header in that request. For more information, see [Preventing Cross-Site Request Forgery (CSRF)](#preventing-cross-site-request-forgery-csrf).
...
<!-- TODO(AS4) insert a link for integration page (for more info on the `expressMiddleware` function) -->

> **Note:** You can't configure your server's CORS behavior if you are using the `startStandaloneServer` function. Instead, you can switch to using the [`expressMiddleware` function](./migration/#migrate-from-apollo-server-express), which does enable you to customize your server's CORS settings. [See below for an example.](#configuring-cors-options-for-apollo-server)
...
If your application doesn't fit into any of the above categories, the default Apollo Server CORS behavior should suit your use case. You can always specify origins in your CORS configuration later or disable cross-origin requests by disabling CORS for your server (e.g., passing `cors: false` to `expressMiddleware`).

## Configuring CORS options for Apollo Server

> **Note:** You can't configure your server's CORS behavior if you are using the `startStandaloneServer` function. Instead, you can switch to using the [`expressMiddleware` function](./migration/#migrate-from-apollo-server-express), which does enable you to customize your server's CORS settings.

By default, Apollo Server's standalone server (i.e., `startStandaloneServer`) and Express integration (i.e., `expressMiddleware`) both serve the `Access-Control-Allow-Origin` HTTP header with the wildcard value (`*`). This allows scripts on any origin to make requests, _without cookies_, to the server and read its responses.
...
The `expressMiddleware` function has a `cors` option, which you can use to change your server's default CORS behavior:
...
Below is an example of setting up CORS with the [`expressMiddleware`](../migration) function:
...
### Disabling CORS or CSRF prevention

## Preventing Cross-Site Request Forgery (CSRF)

> If you want to disable this recommended security feature, pass `csrfPrevention: false`.
...
It should have no impact on legitimate use of your graph _except_ in these two cases:
