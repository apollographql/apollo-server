...
Apollo Server's default CORS behavior enables any website on the Internet to tell browsers to connect to your Apollo Server.

> ⚠️ If your app is only visible on a private network and uses network separation for security, the default CORS behavior is **not secure**. See [Specifying origins in production](#specifying-origins-in-production) for more information.

By default, websites running on domains other than your Apollo Server's own domain won't be able to pass cookies with their requests.  If you want to allow specific websites to connect to your Apollo Server using cookies for authentication, see [Passing Credentials with CORS](#passing-credentials-with-cors).
...
By default, the batteries-included `apollo-server` package serves a special `access-control-allow-headers: *` HTTP header which allows scripts on any origin to make requests to the server and read the responses. The scripts are not allowed to ask the browser to include the user's cookies with the requests.  We'll go into more detail about what this means below.
...
If you create a public API or an API to embed in websites you don't control yourself, you probably want to allow _all_ origins to access your server's resources. You can use the default wildcard value (`*`) in these situations, allowing requests from any origin.
...
If your browser is running your API on a private network (i.e., not on the public internet), **the default behavior of Apollo Server is insecure** and you should specify specific origins. Otherwise, any time your computer is connected to your private network, a script on any website can make your browser talk to the private API. While [some browsers such as Chrome have features under development](https://wicg.github.io/private-network-access/) to solve this problem, it is safest to ensure that servers on private networks do not return `access-control-allow-origin: *`.

## Passing credentials with CORS

If you need websites with origins other than your Apollo Server's origin to [include a user's credentials](https://developer.mozilla.org/en-US/docs/Web/API/Request/credentials) (e.g., cookies or HTTP `Authentication` headers), you will need to modify your CORS configuration to inform browsers that they are allowed to include these credentials.
...
> You *must* specify an origin to enable credentialed requests (i.e., if your server sends the `*` wildcard value then browsers will still refuse to send credentials). The `cors` middleware `origin` option accepts a boolean value, which means you can technically configure CORS to allow all cross-origin requests _with_ credentials (i.e., `{origin: true, credentials: true}`). In production, this is almost certainly insecure, since it means that any website can read information from your server that is protected by the user's cookies. Instead, if you enable credentials you should [specify specific origins in your CORS configuration](#configuring-origins).
