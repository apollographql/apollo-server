...
Depending on what you are building, your CORS configuration might need to change.

For example, if you create a public API or an API to embed in other websites, you probably want to allow all origins to access your server's resources. In these situations, you might want to use the wildcard value (`*`) as your `Access-Control-Allow-Origin` header, allowing requests from any origin.

If you are exposing an API on the public internet specifically to serve your _own_ website or app, then it might be a good idea to specify origins in your CORS config.
<!-- WORK ON THIS! -->
If you do choose to use the default CORS behavior in this scenario (i.e., using the wildcard value (`*`) as your `Access-Control-Allow-Origin` header) then you know that a request can come in with credentials, so you will probably be fine.

If you are passing credentials to your API, then we strongly recommend specify which origins can access your server's resources.

> If you do use the wildcard value in production, _any_ website can tell a user's browser to send an arbitrary request (without cookies or other credentials) to your server and read that server's response.

<!--  Clean this up: Serve your own sites then might be good idea. If have credentials def good idea  -->

### Building private API
<!-- Flesh this out -->
If you have an API that relies on network level controls for security, i.e., your browser is running the API on your private network, we strongly recommend setting specific origins.
