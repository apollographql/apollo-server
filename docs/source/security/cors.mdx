...
> 📣 By default, Apollo Server 4 ships with a feature protecting from CSRF and XS-Search attacks. This feature requires that any client sending operations via `GET` or multipart upload requests _must_ include a non-empty `Apollo-Require-Preflight` (or another non-special-cased) header in that request. For more information, see [Preventing Cross-Site Request Forgery (CSRF)](#preventing-cross-site-request-forgery-csrf).
...
If you're building a [federated graph](/federation/), we strongly recommend that _all_ of your production subgraph servers _disable_ CORS or limit your subgraph's origins to tools like the [Apollo Studio Explorer](https://www.apollographql.com/docs/studio/explorer/embed-explorer/). Most subgraphs should _only_ allow communication from your gateways, and that communication doesn't involve a browser.
...
<!-- TODO(AS4) add link back in to API page once that is updated -->
...
<!-- TODO(AS4) add links to the api pages for startStandaloneServer and expressMiddleware -->
...
By default, Apollo Server's standalone server (i.e., `startStandaloneServer`) serves the `Access-Control-Allow-Origin` HTTP header with the wildcard value (`*`). This allows scripts on any origin to make requests, _without cookies_, to the server and read its responses. 
...
You can also choose to remove CORS middleware entirely to disable cross-origin requests. If you are using `expressMiddleware`, you can do this by passing `false` to the `cors` function:
...
This is [recommended for subgraphs in a federated graph](/federation/subgraphs/#securing-your-subgraphs).
...
Apollo Server 3.7 introduced a CSRF prevention feature, which can be enabled in versions of Apollo Server 3.7+ by passing `csrfPrevention: true` to the `ApolloServer` constructor.
...
For example, if you use the [`apollo-upload-client`](https://github.com/jaydenseric/apollo-upload-client) package with Apollo Client Web, pass the header: `{'Apollo-Require-Preflight': 'true'}` to `createUploadLink`.
