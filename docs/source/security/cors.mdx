...
When thinking about configuring CORS for your application, there are two main options you might want to change: the origins that can access your server's resources and whether your server accepts user credentials (i.e., cookies) with requests.
...
> You *must* [specify an origin](#configuring-cors-options-for-apollo-server) to enable credentialed requests (i.e., if your server sends the `*` wildcard value for the `Access-Control-Allow-Origin` HTTP header, your browser will refuse to send credentials).
