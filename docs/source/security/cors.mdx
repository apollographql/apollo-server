...
> ⚠️ New in Apollo Server 3.7: we **highly recommend** that all users pass `csrfPrevention: true` to `new ApolloServer()` to protect your server from CSRF and XS-Search attacks. If you enable this feature, any clients that send operations via `GET` requests must include a non-empty `Apollo-Require-Preflight` (or any other non-special-cased) header in that request. If you integrate `graphql-upload` into your server, your upload client must also include this header. For more information, see [Preventing Cross-Site Request Forgery (CSRF)](#preventing-cross-site-request-forgery-csrf). Additionally, if you integrate `graphql-upload` into your server and enable `csrfPrevention: true`, your upload client must include a non-empty `Apollo-Require-Preflight` (or any other non-special-cased) header as part of the request. Without this header, the request won't be preflighted and Apollo Server will reject it with a helpful error message.
