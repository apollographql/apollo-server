...
By default, websites running on domains that differ from your server's domain won't be able to pass cookies with their requests.  To enable specific websites to connect to your server using cookies for authentication, see [Passing Credentials with CORS](#passing-credentials-with-cors).
...
## Choosing your CORS configuration
...
### Choosing the CORS origin config for your project
Depending on what you are building, your CORS configuration might need to change to suit what is being built.

#### Private Network
If your browser is running your API on a private network (i.e., not on the public internet), **the default CORS behavior of Apollo Server is insecure**. We strongly recommend specifying which origins can access your server's resources.
...
> If you use the default wildcard value (`*`) in production, _any_ website can tell a user's browser to send an arbitrary request (without cookies or other credentials) to your server and read that server's response.

#### API to serve own App
...
Using [`cors`](https://github.com/expressjs/cors#configuration-options) middleware, you can configure the `Access-Control-Allow-Origin` header using the `origin` option.

By default, the batteries-included `apollo-server` package serves a  `Access-Control-Allow-Origin: *` HTTP header which allows scripts on any origin to make requests _without cookies_ to the server and read its responses. We'll go into more detail about what this means below.
...
If your cross-origin request [includes a user's credentials](https://developer.mozilla.org/en-US/docs/Web/API/Request/credentials) (e.g., cookies or HTTP `Authentication` headers), you need to modify your CORS configuration to tell the browser those credentials are allowed.
...
> You *must* specify an origin to enable credentialed requests (i.e., if your server sends the `*` wildcard value then browsers will still refuse to send credentials).
>
