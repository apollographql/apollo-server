...
The [`startStandaloneServer` function](../migration#migrate-from-apollo-server)'s CORS settings enable any website on the internet to tell a user's browser to connect to your server. However, `startStandaloneServer` doesn't support configuring its CORS behavior. If you want to [customize your CORS behavior](#configuring-cors-options-for-apollo-server), you'll need to swap to using [`expressMiddleware`](../migration#migrate-from-apollo-server-express) (or any other Apollo Server integration). 
