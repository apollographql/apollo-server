...
In development, Apollo Server enables GraphQL Playground on the same URL as the GraphQL server itself (e.g. `http://localhost:4000/graphql`) and automatically serves the GUI to web browsers.  When `NODE_ENV` is set to `production`, GraphQL Playground (as well as introspection) is disabled as a production best-practice. It does this by installing the `ApolloServerPluginFrontendGraphQLPlayground` plugin with default settings.

![GraphQL Playground](../images/graphql-playground.png)

## Configuring Playground

If you'd like to configure playground, you can add `ApolloServerPluginFrontendGraphQLPlayground` to your Apollo Server yourself. The options can be found on GraphQL Playground's [documentation](https://github.com/graphql/graphql-playground/#usage).
...
If you don't want to run GraphQL Playground at all, you have two choices. First, you can replace it with a different frontend by installing a plugin that implements the [`renderFrontend` hook](../integrations/plugins/#renderfrontend). Alternatively, you can install the `ApolloServerPluginFrontendDisabled` plugin which stops Apollo Server from serving any HTML frontend.

```js
const { ApolloServer } = require('apollo-server');
const { ApolloServerPluginFrontendDisabled } = require('apollo-server-core');
