...
Apollo Server uses a [multi-step request pipeline](/apollo-server/integrations/plugins/#request-lifecycle-event-flow) to validate and execute incoming GraphQL operations. This pipeline supports integration with custom plugins at each step, which can affect an operation's execution. Because of this, it's important to perform **integration tests** with a variety of operations to ensure your request pipeline works as expected.
...
<!-- TODO(AS4) add link once the API article is updated -->
The `executeOperation` method accepts the following arguments:
...
- An optional second object that is used as the operation's [context value](../data/resolvers/#the-context-argument)).
...
Note that when testing, any errors in parsing, validating, and executing your If you are using the `startStandaloneServer` function you don't need to start your server before calling `executeOperation`. The server instance will start automatically and throw any startup errors. If you are using an Apollo Server integration (e.g., `expressMiddleware`) you do need to start your server before calling the `executeOperation` method.GraphQL operation are returned in the `errors` field of the operation result. As with any GraphQL response, these errors are not _thrown_.
...
> ⚠️ The Apollo Server 4 alpha doesn't currently support using [`RESTDataSource`](../migration#datasources), a class commonly used to fetch data from a database or a REST API. This feature is in active development, but in the meantime the below code snippet doesn't work as described.
...
The example above includes a test-specific [context value](../data/resolvers/#the-context-argument), which provides data directly to the `ApolloServer` instance, bypassing any `context` initialization function you have.
...
<!-- TODO(AS4) double check if this package is going to be updated for AS4 -->
<!-- There are also community packages available such as [`apollo-server-integration-testing`](https://www.npmjs.com/package/apollo-server-integration-testing), which uses mocked Express request and response objects. -->
