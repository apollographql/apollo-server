...
- An optional second object that is used as the operation's [`context`](../data/resolvers/#the-context-argument) value.
...
Unlike with a normal instance of `ApolloServer`, you don't need to start your server (e.g., using `startStandaloneServer` or `expressMiddleware`) before calling `executeOperation`. The server instance will start automatically and throw any startup errors.
...
The example above includes a test-specific [`context`](../data/resolvers/#the-context-argument) value, which provides data directly to the `ApolloServer` instance, bypassing any `context` initialization function you have.
