...
There are also community packages available such as [`@apollo/server-integration-testsuite`](https://www.npmjs.com/package/@apollo/server-integration-testsuite), which provides a suite of Jest tests that is useful for [integration authors](../integrations/building-integrations).
