...
If you are using the `startStandaloneServer` function you don't need to start your server before calling `executeOperation`. The server instance will start automatically and throw any startup errors. If you are using an Apollo Server integration (e.g., `expressMiddleware`) you do need to start your server before calling the `executeOperation` method.
