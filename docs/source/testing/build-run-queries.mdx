...
You can serve a custom HTML landing page from Apollo Server's base URL. To do so, define your own [custom plugin](/apollo-server/integrations/plugins/) that calls the [`renderLandingPage` method](/apollo-server/integrations/plugins-event-reference/#renderlandingpage), like so:
