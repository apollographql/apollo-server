...
To get the most out of GraphQL, your organization should expose a [single data graph](https://principledgraphql.com/integrity#1-one-graph) that provides a unified interface for querying any combination of your backing data sources. However, it can be challenging to represent an enterprise-scale data graph with a single, monolithic GraphQL server. To remedy this, you can divide your graph's implementation across multiple composable services with **Apollo Federation**.

Unlike other distributed GraphQL architectures (such as schema stitching), Apollo Federation uses a declarative programming model that enables each **implementing service** to implement _only_ the part of your graph that it's responsible for.

Apollo Federation also supports a [managed mode with Apollo Studio](https://www.apollographql.com/docs/studio/managed-federation/overview/), which enables you to add, remove, and refactor your implementing services without requiring any downtime for your production graph. Managed federation is a free Studio feature that does not require a paid plan.
...
Consider the `recentPurchases` field of the `User` type in the above schema. Even though this field is a member of the `User` type, a list of `Product`s should probably be populated by the `products` service, _not_ the `users` service.
