...
To help with debugging, Apollo Server provides a `ApolloServerErrorCode` enum that you can use to check if your error is one of the [different types recognized by Apollo Server](#error-codes).
...
You can also [create your own custom errors and codes](#custom-errors), enabling requesting clients to respond differently to different error types.
...
The request was parsed successfully and is valid, but the server couldn't resolve which operation to run.

This error occurs when a request contains multiple named operations without specifying which operation to run, or if the specified operation isn't included in the request.
