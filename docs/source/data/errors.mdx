...
For example, if you are using a database library in your app and you'd like your server to do something when it throws a specific type of database error:
...
However, if a _resolver_ throws the error, a `GraphQLError` is wrapped around that initially thrown error.

The `unwrapResolverError` function either removes the `GraphQLError` wrapping from a resolver error or returns the error unaltered if it isn't from a resolver.
...
If you _do_ want error information sent to Studio, you can choose to send to either send all errors unmodified or you can modify or redact specific errors before they're transmitted:
...
[As mentioned above](#for-client-responses), you can use the `unwrapResolverError` (from `@apollo/server/errors`) to remove the `GraphQLError` wrapping the original error.

<!-- TODO(AS4) Replace link once plugin docs are updated -->

> **For federated graphs**, define your `transform` function in each subgraph's [inline trace plugin](./migration/rewriteEerror-plugin-option) to rewrite field errors. If you want to transform your gateway's parsing or validation errors, you can define your `transform` function in your gateway.
