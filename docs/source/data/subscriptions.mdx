...
>
> This article has been updated to use the `graphql-ws` library. We strongly recommend against using `subscriptions-transport-ws`, as this library is not longer being actively maintained. The [previous version of this article can be found here](https://github.com/apollographql/apollo-server/blob/4b82967c10a69aa104848a8c9d15c591b528918c/docs/source/data/subscriptions.mdx). If you plan to sure subscriptions with Apollo Client, ensure you setup [for the subscription protocol you are using](https://www.apollographql.com/docs/react/data/subscriptions/#choice-of-subscription-protocol).
>
...
To run both an Express app _and_ a separate subscription server (using ws's WebSocketServer), we'll create an `http.Server` instance that effectively wraps the two and becomes our new `listen`er.
