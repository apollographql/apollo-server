...
This article details how to set up the [`graphql-ws`](https://www.npmjs.com/package/graphql-ws) library to add support for subscriptions to Apollo Server. Previously this article recommended using the `subscriptions-transport-ws` library; however, this library is no longer actively maintained.

Apollo Server can be combined with both `graphql-ws` and `subscriptions-transport-ws`, but we strongly recommend using the currently maintained `graphql-ws` library.

If you intend to switch from `subscriptions-transport-ws` to `graphql-ws` you will need to:
* [Adjust Apollo Studio Explorer settings](https://www.apollographql.com/docs/studio/explorer/additional-features/#subscription-support)
* Migrate to [Apollo Client 3.6 or later](https://www.apollographql.com/docs/react/data/subscriptions/#choice-of-subscription-protocol)
  * Replace usage of [`WebSocketLink` with `GraphQLWsLink` ](https://www.apollographql.com/docs/react/api/link/apollo-link-subscriptions/).
* [Customize your WebSocket protocol](https://www.apollographql.com/docs/kotlin/essentials/subscriptions/#customizing-your-websocket-protocol) if using Apollo Kotlin

<!-- TODO: This may need updating depending on whether iOS supports or not.  -->
Note that [Apollo iOS](https://www.apollographql.com/docs/ios/) does not currently support using the  `graphql-ws` library.
