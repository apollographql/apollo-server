...
You could replicate this behavior by providing a `context` option to the first argument passed to the `useServer` function.

In the example below, we check a client's authentication every time they send a `subscription` operation. Then if that user exists, we add the current user to our GraphQL operation context:
