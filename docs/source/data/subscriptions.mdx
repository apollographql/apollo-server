...
>
> This article has been updated to use the `graphql-ws` library with Apollo Server. We no longer recommend using the previously documented `subscriptions-transport-ws`, because this library is no longer actively maintained. For more information, see [The old `subscriptions-transport-ws` library](#the-old-subscriptions-transport-ws-library).
...
5. Create a `WebSocketServer` to serve as our subscription server.

    ```ts:title=index.ts
      // This will be the websocket server we use for our subscriptions
      ...
      // Hand in the schema we just create and start the WebSocketServer
...
After you start up the server in CodeSandbox, follow the instructions in the browser to test out running the `numberIncremented` subscription in Apollo Sandbox. You'll see the subscription's value update every second.

> Note, if you run into problems you might need to [adjust your Sandbox settings to use the `graphql-ws` protocol](https://www.apollographql.com/docs/studio/explorer/additional-features/#subscription-support).
...
>If you plan to use subscriptions with Apollo Client, ensure your subscription protocol [is consistent with the subscription library you are using](https://www.apollographql.com/docs/react/data/subscriptions/#choice-of-subscription-protocol).
...
## The old `subscriptions-transport-ws` library

This article details hows to setup the [`graphql-ws`](https://www.npmjs.com/package/graphql-ws) library with Apollo Server. Previously this article recommended using the `subscriptions-transport-ws` library, however this library is no longer actively maintained.

The two libraries implement different protocols for GraphQL subscriptions over websockets. If you do decide to switch from one library to another you will need to ensure your [Apollo Client subscription protocol](https://www.apollographql.com/docs/react/data/subscriptions/#choice-of-subscription-protocol) is coordinated with the subscription library you are using. Note that you may also need to [adjust the Apollo Sandbox settings to use the subscription protocol for the library you are using](https://www.apollographql.com/docs/studio/explorer/additional-features/#subscription-support).

You can find the [previous version of this article using `subscriptions-transport-ws` here](https://github.com/apollographql/apollo-server/blob/4b82967c10a69aa104848a8c9d15c591b528918c/docs/source/data/subscriptions.mdx).
