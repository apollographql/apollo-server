...
**For subscriptions,** you provide an `onConnect` function to the `SubscriptionServer` constructor. This function receives the [`connectionParams`](#connectionParams) in an object as the first argument, as well as the [`WebSocket`](#websocket) and [`ConnectionContext`](#context) as the second and third arguments, respectively. If your `onConnect` function returns an object, it will be passed to your resolvers as the `context` argument.
