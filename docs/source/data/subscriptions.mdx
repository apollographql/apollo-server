...
> The following steps assume you've already [swapped to using the `expressMiddleware` function](./migration#migrate-from-apollo-server-express).
...
4. Add a [plugin](/apollo-server/integrations/plugins) to your `ApolloServer` constructor to properly shutdown your new `WebSocketServer`:
