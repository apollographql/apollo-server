...
**For subscriptions,** you provide an `onConnect` function to the `SubscriptionServer` constructor. This function receives the HTTP headers in an object as the first argument, as well as the [`WebSocket`](#websocket) and [`ConnectionContext`](#context) as the second and third arguments, respectively. If your `onConnect` function returns an object, it will be passed to your resolvers as the `context` argument.
...
* If `onConnect` returns an object, that object will be passed as the _operation_ `context` to your resolvers.
...
The `connectionParams` argument in the `SubscriptionServer`'s `onConnect` callback will contain the information passed by the client and can be used to validate user credentials.

From the `onConnect` function, we can then return an object which will be passed into our resolvers as the `context` argument during execution. For our example, we can use the `authorization` token provided by the client to lookup the related user and pass that user along to our resolvers.
