...
> ⚠️ **Running into an error?** If you are using the `graphql-ws` library your specified subscription protocol must be consistent across your backend, frontend and *every other tool* you use. For more information see [Switching from `subscriptions-transport-ws`](#switching-from-subscriptions-transport-ws).
  ...
  Subscription: {
    hello: {
      ...
      subscribe: async function* () {
        for await (const word of ["Hello", "Bonjour", "Hola", "Ciao", "Zdravo"]) {
          yield { hello: word };
        }
      },
    },
...
The `subscribe` function **must** return an object of type `AsyncIterator`, a standard interface for iterating over asynchronous results. In the example `postCreated.subscribe` field above, an `AsyncIterator` is generated by `pubsub.asyncIterator` (more on this below).
