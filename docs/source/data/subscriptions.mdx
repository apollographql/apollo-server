...
This article previously demonstrated using the `subscriptions-transport-ws` library to set up subscriptions. However, this library is no longer actively maintained. You can still use it with Apollo Server, but we strongly recommend using [`graphql-ws`](https://www.npmjs.com/package/graphql-ws) instead. A migration path from `subscriptions-transport-ws` to `graphql-ws` is outlined in the [Apollo Server v3 migration guide](/migration/#subscriptions).
...
<!-- How to change from using subscriptions-transport-ws to using graphql-ws -->
<!-- 1. install  -->
...
3. Create a `WebSocketServer` to use as your subscription server. This will replace the `SubscriptionServer` created by `subscriptions-transport-ws`.
...
4. Add [plugins](../integrations/plugins) to your `ApolloServer` constructor to shutdown the `WebSocketServer`:
...
5. Finally, ensure you are `listen`ing to your `httpServer`.

  Last step, make sure everything works before you uninstall `subscriptions-transport-ws`
