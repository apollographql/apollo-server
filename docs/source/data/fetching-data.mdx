...
export class BookingsDataSource {
  ...
  async getUser() {
    if (!this.user) {
      // cache the user, lookup by token
...
- If your data source class uses in memory caching of values that shouldn't be shared across requests, or its constructor requires an argument, the `context` function should create a _new instance_ of each data source class for each operation.
