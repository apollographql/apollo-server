...
3. RESTDataSource has a response cache that provides caching when fetching from rest
...
To make any of the Above Apollo Server 3 data sources work you have to first construct the data source before calling `initialize` and passing in Apollo Server's cache, like so:
