...
Because your server can use various data sources, keeping your [resolvers](./resolvers) tidy becomes even more important. For this reason, we recommend creating data source classes to encapsulate the logic of fetching data from a particular source, then providing methods that resolvers can use to fetch data neatly.

## Creating data source classes

> [In Apollo Server 3](/apollo-server/v3/data/data-sources), you'd subclass the `DataSource` abstract class for each of your data sources. In Apollo Server 4, we recommend [making a custom class for each data source](#creating-data-source-classes), each class best suited for that particular store or API.
...
Below is simple class for connecting to a built-in static data set:

<MultiCodeBlock>

```ts title="datasources.ts"
...
Check out [`RestDataSource`](https://github.com/apollographql/datasource-rest/blob/main/src/RESTDataSource.ts) class for an example of a data source class that handles caching, deduplication, and errors.
...
const { url } = await startStandaloneServer(server, {
  context: async () => {
    ...
    return {
      ...
      // We can pass in our server's cache, context, or any other
      // info our data sources require. 
      // highlight-start
      dataSources: {
        dogsDB: new DogsDataSource({ cache, context: token }),
...
const resolvers = {
  Query: {
    dogs: async (_, { id }, { dataSources }) => {
      return dataSources.dogsDB.getDogs();
    },
...
In Apollo Server 3, immediately after constructing each `DataSource` subclass, your server would invokes the `initialize({ cache, context })` method on each new `DataSource`. 
...
class BooksAPI extends HTTPDataSource {
  ...
  constructor(options: { cache: KeyValueCache<string>; context: string }) { //highlight-line
    // the necessary arguments for HTTPDataSource
    const pool = new Pool(baseURL);
    super(baseURL, { pool });

    // We need to call the initialize method in our data source's
    // constructor, passing in our cache and context.
...
const { url } = await startStandaloneServer(server, {
  context: async () => {
    ...
    return {
      dataSources: {
        moviesApi: new MoviesAPI({ cache, context: token }),
      },
    //highlight-end
      token 
    };
