...
When dataSources are initialized, the initialize method is called. This is useful when you need to initialize the dataSources with the context or cache.

This is the code that initializes the dataSources.
...
If you need to initialize the dataSources with the context or cache, you can use the initialize method.

```js
class InternalApi extends RESTDataSource {
  constructor() {
    super();
    this.baseURL = 'https://internal-api.example.com/';
  }

  async initialize({ context, cache }) {
