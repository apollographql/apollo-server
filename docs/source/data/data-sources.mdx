...
Apollo Server supports using [Memcached](https://memcached.org/) or [Redis](https://redis.io/) (as well as other cache backends) via the [`keyv`](https://www.npmjs.com/package/keyv) package. Visit [Configuring external caching](../performance/cache-backends#configuring-external-caching) for more information and examples on configuring Apollo Server's cache.

You can also implement your own cache backend. Visit [Implementing your own cache backend](../performance/cache-backends#implementing-your-own-cache-backend) for more information on implementing the `KeyValueCache` interface.
