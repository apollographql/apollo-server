...
A "malicious" client could provide an `:id` of `1/characters` to target the `characters` endpoint when it was the singular `movie` endpoint that we were expecting. URI encoding prevents this kind of injection by transforming the `/` into `%2F`. This can then be correctly decoded and interpreted by the server and won't be treated as a path segment.
