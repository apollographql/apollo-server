...
The `context` function should be _asynchronous_ and return an object, which is then accessible to your server's resolvers and plugins (via the `contextValue` field).
