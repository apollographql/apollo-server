...
Resolver functions take the following positional arguments, in order. (By "positional" we mean that these arguments are passed as individual arguments to your resolver; you can give them whatever name you want. Our documentation generally uses the names given below, though it's also common to use the type's name or `source` instead of `parent` for the first argument.)
