...
> To learn how to fetch data from a REST API, see [Fetching from REST](./fetching-rest/). To learn how to fetch data from other sources, see [Fetching Data](./fetching-data).
...
The `context` argument is useful for passing things that any resolver might need, like [authentication scope](https://blog.apollographql.com/authorization-in-graphql-452b1c402a9), [sources for fetching data](./fetching-data/), database connections, and custom fetch functions. If you're using [dataloaders to batch requests](./fetching-rest/#using-with-dataloader) across resolvers, you can attach them to the `context` as well.
