...
Here's a code sample that can resolve the query above with this resolver chain (note that it requires the `lodash` library in addition to `apollo-server`):
...
const resolvers = {
  ...
  Library: {
    books(parent) {
      ...
      return filter(books, { branch: parent.branch });
    }
  },
  Book: {
...
**Resolvers should never modify the `context` argument.** This ensures consistency across all resolvers and prevents unexpected errors.
