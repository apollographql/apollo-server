...
You can extend the `RESTDataSource` class to implement whatever data-fetching methods your resolvers need. These methods should use the built-in convenience methods (e.g., `get` and `post`) to perform HTTP requests, helping you add query parameters, parse and cache JSON results, dedupe requests, and handle errors. More complex use cases can use the `fetch` method directly. The `fetch` method returns both the parsed body as well as the response object which affords more flexibility for use cases like reading response headers.
...
- The first layer deduplicates _concurrent_ outgoing `GET` (and `HEAD`) requests by default. Deduplication is keyed on the request's method and URL. This behavior can be configured by overriding the `requestDeduplicationPolicyFor` method. For more details on how to configure this please consult [the README](https://github.com/apollographql/datasource-rest#requestdeduplicationpolicyfor).

> Note: in versions prior to v5 of `RESTDataSource`, _all_ outgoing `GET` requests were deduplicated. This behavior can be restored using the `deduplicate-until-invalidated` policy (explained more in-depth in [the README](https://github.com/apollographql/datasource-rest#requestdeduplicationpolicyfor)).
...
You can change how `GET` (and `HEAD`) requests are stored in `RESTDataSource`'s deduplication cache by overwriting the [`cacheKeyFor` method](https://github.com/apollographql/datasource-rest#cachekeyfor). By default, the cache key is the method and URL of the request.

If you'd like to restore the deduplication policy from before `RESTDataSource` v5, you can configure `requestDeduplicationPolicyFor` like so:
...
To disable request dedupliation entirely, you can configure `requestDeduplicationPolicyFor` like so:
