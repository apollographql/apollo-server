...
Apollo Server 3 provided a GraphQL Playground landing page by default. For migration purposes, we have published the [`@apollo/plugin-landing-page-graphql-playground`](https://www.npmjs.com/package/@apollo/plugin-landing-page-graphql-playground) package, which is a GraphQL Playground plugin that is compatible with Apollo Server 4. However, we do not intend to support this plugin via documentation, security updates, or otherwise. Instead, we recommend migrating to the actively maintained Apollo Sandbox (the default landing page in Apollo Server 4) at your earliest convenience.
