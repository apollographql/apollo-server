...
To configure these default plugins while still using the same `NODE_ENV`-based logic, import them from the `@apollo/server/plugin/landingPage/default` package and pass them to the `ApolloServer` constructor in the `plugins` array:
...
> ⚠️ The GraphQL Playground project is [**retired**](https://github.com/graphql/graphql-playground/issues/1143). Instead, we recommend using Apollo Sandbox, Apollo Server's [default development landing page](#default-non-production-landing-page).
