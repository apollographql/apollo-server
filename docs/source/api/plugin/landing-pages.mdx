...
If you aren't using the embedded Apollo Sandbox (i.e., you passed in `{embed: false}`), the landing splash page displays a footer that links to the documentation telling you how to configure it. To remove this footer, pass `footer: false`.
...
> **Note:** The GraphQL Playground project is [retired](https://github.com/graphql/graphql-playground/issues/1143). Instead, we recommend using Apollo Sandbox, Apollo Server's [default development landing page](#default-non-production-landing-page). 
...
The GraphQL Playground plugin lives in its own `@apollo/server-plugin-landing-page-graphql-playground` package. To use it, install this package and provide the plugin to the `ApolloServer` constructor:
