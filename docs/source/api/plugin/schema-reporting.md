...
This plugin was introduced in Apollo Server FIXME(no-engine). In previous versions, schema reporting was configured using the `engine` option to the `ApolloServer` constructor. That option continues to work; see migration guide FIXME(no-engine) for details.
