...
In certain cases, Apollo Server installs some of its built-in plugins automatically (for example, when you provide an Apollo Studio API key with the `APOLLO_KEY` environment variable). For details, see the API references for these plugins: [usage reporting](../plugin/usage-reporting/), [schema reporting](../plugin/schema-reporting/), [landing page](../api/plugin/landing-pages/), and [inline trace](../plugin/inline-trace/).
