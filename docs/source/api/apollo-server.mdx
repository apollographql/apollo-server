...
An object that specifies options for GraphQL validation. This object is passed directly as the `options` argument to the GraphQL `validate` function. See the [GraphQL-JS documentation for `validate`](https://www.graphql-js.org/api-v16/validation/#validate).
