...
An array of [plugins](/apollo-server/integrations/plugins) to install in your server instance. Each array element can be either a valid plugin object or a zero-argument function that _returns_ a valid plugin object.

In certain cases, Apollo Server installs some of its built-in plugins automatically (for example, when you provide an Apollo Studio API key with the `APOLLO_KEY` environment variable). For details, see the API references for these plugins: [usage reporting](/apollo-server/plugin/usage-reporting/), [schema reporting](/apollo-server/plugin/schema-reporting/), [landing page](/apollo-server/api/plugin/landing-pages/), and [inline trace](/apollo-server/plugin/inline-trace/).
...
2. Your server calls all of the [`serverWillStart` handlers](/apollo-server/integrations/plugins-event-reference/#serverwillstart) of your installed plugins. If any of these handlers throw an error, `start` throws an error.


## `startInBackgroundHandlingStartupErrorsByLoggingAndFailingAllRequests`

**Serverless** integrations handle starting an Apollo Server instance in the background by calling the  `startInBackgroundHandlingStartupErrorsByLoggingAndFailingAllRequests` method. See [Building integrations](../integrations/building-integrations#ensure-successful-startup) for more details. 

This method [triggers the same actions](#triggered-actions) at the `start()` method.

## `executeHTTPGraphQLRequest`

You can use the `executeHTTPGraphQLRequest` method to execute a GraphQL request by passing in an [`HTTPGraphQLRequest`](./integrations/building-integrations/#construct-the-httpgraphqlrequest-object) object.
...
* The second optional argument is used as the operation's [context value](../data/resolvers/#the-context-argument). Note, this argument is **not optional** if your server uses a context function.  
...
<!-- TODO(AS4) Make this better -->
This field contains information about a request's extensions. 
...
An object implementing the [`HTTPGraphQLRequest`](https://github.com/apollographql/apollo-server/blob/2d5d564abe3d817f8902f07a712a6e105f1b2e41/packages/server/src/externalTypes/http.ts#L2) interface, mimicking the information that would in a real HTTP request. 
</td>
</tr>
