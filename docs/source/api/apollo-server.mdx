...
You can pass a [`HTTPGraphQLRequest`](./integrations/building-integrations/#construct-the-httpgraphqlrequest-object) object to the `executeHTTPGraphQLRequest` method to execute a GraphQL request:
...
The `extensions` object is usually used to pass information within your server from one touch point to another (e.g., from a request to a plugin). 
...
An object implementing the [`HTTPGraphQLRequest`](https://github.com/apollographql/apollo-server/blob/2d5d564abe3d817f8902f07a712a6e105f1b2e41/packages/server/src/externalTypes/http.ts#L2) interface, mimicking the information that would be in a real HTTP request. 
</td>
</tr>
