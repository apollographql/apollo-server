...
You can use the [`context` function](../data/resolvers/#the-context-argument) to get information about the current operation from the original Lambda data structures. Your `context` function can access this information from the passed-in `event` and `context` arguments:
...
If you've [changed your set up to use `@vendia/serverless-express`](#customizing-http-behavior) you'll also have access to an `express` argument, containing the Express request and response in your `context` function:
