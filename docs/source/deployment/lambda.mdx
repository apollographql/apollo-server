...
The `@as-integrations/aws-lambda` package is compatible with Lambda's API Gateway V1 (REST) and V2 (HTTP). However, it does not let you customize HTTP behavior directly or support other AWS products that invoke Lambda functions (e.g., such as Application Load Balancers).
...
You can use the [`context` function](../data/resolvers/#the-context-argument) to get information about the current operation from the original Lambda data structures.
