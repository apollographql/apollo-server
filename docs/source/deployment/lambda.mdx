  ...
  graphql:
    ...
    events:
      ...
      - http:
          ...
          method: get
          cors: true
# Omit the below lines if you aren't using TS!
plugins:
  - serverless-plugin-typescript
```

> If you are using webpack, you might want to install and configure Serverless using the [`serverless-webpack` plugin](https://github.com/serverless-heaven/serverless-webpack) to ensure the CLI can find your file.
...
The `@as-integrations/aws-lambda` package is compatible with Lambda's API Gateway V1 (REST) and V2 (HTTP). However, it does not let you customize HTTP behavior directly.

If you'd like to customize your HTTP behavior, you can couple Apollo Server's Express integration (i.e., [`expressMiddleware`](../api/express-middleware)) with the [`@vendia/serverless-express`](https://github.com/vendia/serverless-express) package. The `@vendia/serverless-express` library translates between Lambda events and Express requests.
...
You can use the [`context` function](../data/resolvers/#the-context-argument) to get information about the current operation from both the original Lambda data structures. These are provided to your `context` function as `event` and `context` arguments.

The `event` object contains the API Gateway event (HTTP headers, HTTP method, body, path, etc.). The `context` object (not to be confused with the `context` function itself!) contains the current Lambda Context (function name, function version, awsRequestId, time remaining, etc.).
...
If you've [changed your set up to use `@vendia/serverless-express`](#customizing-http-behavior) you'll also have access to an `express` argument, containing the Express request and response.
