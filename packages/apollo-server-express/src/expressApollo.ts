...
export function graphiqlExpress(options: GraphiQL.GraphiQLData | ExpressGraphiQLOptionsFunction) {
  ...
  return (req: express.Request, res: express.Response, next) => {
    if (req.path.includes(`/assets`)) {
      return res.sendFile(path.basename(req.url), { root: publicPath }, err => {
        if (!err) return;
        res
          .status(404)
          .send(`GET ${req.originalUrl} failed.`)
          .end();
      });
    }
    ...
    GraphiQL.resolveGraphiQLString(req.originalUrl, query, options, req).then(
      graphiqlString => {
