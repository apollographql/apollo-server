		...
		return (context: HttpContext, req: FunctionRequest) => {
			if (cors && cors.origin) {
				if (typeof cors.origin === 'string') {
					...
					(Array.isArray(cors.origin) && cors.origin.includes(req.headers['Origin'] || req.headers['origin']))
				) {
					corsHeaders['Access-Control-Allow-Origin'] = req.headers['Origin'] || req.headers['origin'];
				}

				if (!cors.allowedHeaders) {
					corsHeaders['Access-Control-Allow-Headers'] = req.headers['Access-Control-Request-Headers'];
				}
			}
