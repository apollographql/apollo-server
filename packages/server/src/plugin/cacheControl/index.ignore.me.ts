...
import { VariableValues } from '../../externalTypes/graphql.js';
    ...
    async requestDidStart() {
      ...
      const fieldPolicies: {
        [key: string]: {
          ...
          targetType: GraphQLNamedOutputType;
        };
      } = {};

      return {
        async executionDidStart(requestContext) {
          let { document, schema, request } = requestContext;
          let { variables } = request;

          // Collect all the fragments that may have been sent along
          ...
          // Find our Root type for our operation (query, mutation, or subscription)
          const rootOperation = document.definitions.find(
            (def) => def.kind === 'OperationDefinition',
          ) as OperationDefinitionNode;
          const rootType =
            rootOperation.operation === 'query'
              ? schema.getQueryType()
              : rootOperation.operation === 'mutation'
                ? schema.getMutationType()
                : schema.getSubscriptionType();

          // Here we are going to find all the fields/types used in the operation.
          let collectedFields = collectFieldsAndTypes(
            schema,
            fragments,
            variables,
            rootType as GraphQLObjectType,
            rootOperation.selectionSet as SelectionSetNode,
          );
          ...
          collectedFields.forEach((field) => {
            ...
            // type.
            const targetType = getNamedType(field.fieldType);
            ...
            const fieldAnnotation = memoizedCacheAnnotationFromField(
              field.parentType.getFields()[field.fieldName],
            );
            ...
            const path = responsePathAsArray(field.path).join('.');
            fieldPolicies[path] = {
              policy: fieldPolicy,
              inheritMaxAge,
              targetType,
...
function collectFieldsAndTypes(
  ...
  fields.forEach((fieldNodes) => {
    fieldNodes.forEach((fieldNode) => {
      ...
      if (fieldNode.selectionSet) {
        collectedFields = collectedFields.concat(
          collectFieldsAndTypes(
            schema,
            fragments,
            variables,
