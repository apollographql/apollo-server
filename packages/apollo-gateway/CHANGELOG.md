...
- Drop support for `DEBUG=apollo-gateway:`, powered by `loglevel-debug` in lieu of new `logger` option.  This also drops the very long date and time prefix in front of each log line.  The existing behavior can be replicated by providing a `logger` that uses `loglevel-debug`, if desired. [PR #3896](https://github.com/apollographql/apollo-server/pull/3896)
