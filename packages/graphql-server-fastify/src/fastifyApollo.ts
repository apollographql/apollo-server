...
export function graphqlFastify(options: GraphQLOptions | FastifyGraphQLOptionsFunction): express.Handler {
  if (!options) {
  ...
  return (req: express.Request, res: express.Response, next: express.NextFunction): void => {
    const q = req.url && url.parse(req.url, true) || {};
    const query = q.query || '';
    runHttpQuery([req, res], {
      method: req.method,
      options: options,
      query: req.method === 'POST' ? req.body : query,
    }).then((gqlResponse) => {
...
export function graphiqlFastify(options: GraphiQL.GraphiQLData) {
