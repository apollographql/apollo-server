...
export function graphiqlRestify(options: GraphiQL.GraphiQLData | RestifyGraphiQLOptionsFunction) {
  ...
  return (req: restify.Request, res: restify.Response, next: restify.Next) => {
    ...
    if (req.url.includes(`/assets`)) {
      ...
      const stream = fs.createReadStream(path.join(publicPath, fileName));

      const extName = path.extname(fileName);
      res.setHeader(`content-type`, extName === `.js` ? `application/javascript` : `text/css`);

      stream.on(`error`, err => {
        error = err;
        res.status(404);
        return res.send(`File ${req.url} not found.`);
      });

      return stream.pipe(res);
    }

    GraphiQL.resolveGraphiQLString(req.url, query, options, req).then(
      graphiqlString => {
