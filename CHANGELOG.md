...
- `apollo-server-core`: The URLs which Apollo Server connects to when reporting usage and schemas to Apollo's servers have changed. If the environment in which you run your servers requires you to explicitly allow connections by domain, you will need to add the new domain names. Usage reporting previously connected to https://engine-report.apollodata.com/ and now connects to https://usage-reporting.api.apollographql.com/; schema reporting previously connected to https://edge-server-reporting.api.apollographql.com/ and now connects to https://schema-reporting.api.apollographql.com/ . [PR #4453](https://github.com/apollographql/apollo-server/pull/4453)

- `apollo-server-core`: The code which configures how Apollo Server talks to Apollo's commercial products has been refactored into three separate plugins (for usage reporting, schema reporting, and inline tracing) which use the request pipeline plugin API and are shipped with `apollo-server-core`. Instead of configuring them via the `engine` option to the `ApolloServer` constructor, you configure them by calling the plugin functions yourself and including them in your servers `plugins` list. The `engine` option continues to work for backwards compatibility, but configuring future features will require a small rewrite to use the plugins instead of the `engine` option. This change is largely backwards compatible, though there are some small changes, including an extra startup log line from the usage reporting and inline trace plugins, and a reduction of startup log lines from the schema reporting plugin. One behavior change is that previously, if your schema is for a federated implementing service, the federated inline tracing plugin (which includes performance traces in responses when requested in an HTTP header) would turn on automatically except when you also provide an Apollo API key. In v2.18, this special case does not exist: the default is to enable federated tracing for any federated implementing service regardless of whether you provide an API key. The new log lines should make this behavior more clear, and you can manually enable or disable inline tracing. The `apollo-engine-reporting` npm package is now obsolete and the `apollo-engine-reporting-protobuf` package has been renamed to `apollo-reporting-protobuf` (neither of these packages were ever supported for direct use). Implementations of `ApolloServer` for serverless frameworks such as Lambda should now override the `serverlessFramework()` method to return true, instead of manually setting a default value for `engine.sendReportsImmediately` in their constructors. The `GraphQLServiceContext` type passed to the plugin `serverWillStart` method now contains `apollo` and `serverlessFramework` values. Full details are available in [the migration guide](https://www.apollographql.com/docs/apollo-server/migration-engine-plugins/). [PR #4453](https://github.com/apollographql/apollo-server/pull/4453)
