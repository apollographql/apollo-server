...
- Apollo Server no longer calls the "end hook" function returned by an `executionDidStart` event. The `executionDidStart` event should now return an object with an async function property called `executionDidEnd`.
